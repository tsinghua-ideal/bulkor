    ...
    Prod<Z, MetaSize>: ArrayLength<u8> + PartialDiv<U8>,
{
    ...
    >(
        ...
        if snapshot_id > 0 {
            ...
            let stash_data_len = ns + 16 + 8 + 4 + stash_size * ValueSize::USIZE;
            let mut stash_data_buf = vec![0 as u8; stash_data_len];
            let stash_meta_len = ns + 16 + 8 + 4 + stash_size * MetaSize::USIZE;
            ...
            s_decrypt(&ORAM_KEY, &mut stash_data_buf, 12);
            s_decrypt(&ORAM_KEY, &mut stash_meta_buf, 12);
            ...
            let iter_data = (&stash_data_buf[(ns + 28)..]).chunks_exact(ValueSize::USIZE);
            assert_eq!(iter_data.remainder(), []);
            stash_data = iter_data
                .into_iter()
                .map(|d| Aligned(GenericArray::clone_from_slice(d)))
                .collect::<Vec<_>>();
            let iter_meta = (&stash_meta_buf[(ns + 28)..]).chunks_exact(MetaSize::USIZE);
        ...
        // TODO: use no_crash to handle the case where it is necessary to
        // read other files when the currently read files are crashed
    ...
    Prod<Z, MetaSize>: ArrayLength<u8> + PartialDiv<U8>,
{
    ...
    fn persist(&mut self, new_snapshot_id: u64, volatile: bool) {
        ...
        s_encrypt(&ORAM_KEY, &mut stash_data, 12, &mut self.rng);
        s_encrypt(&ORAM_KEY, &mut stash_meta, 12, &mut self.rng);
